...
@pytest.mark.skipif(
    os.environ.get("IS_CI_RUNNER") == "True",
    reason="skipping video rendering for CI/CD",
)
def test_render_gif():
    render_gif_cfg_fn = resource_filename(
        __name__, "test_render_files/mvc_render_gif.yaml"
    )
...
def test_render_mp4():
    render_mp4_cfg_fn = resource_filename(
        __name__, "test_render_files/mvc_render_mp4.yaml"
    )
