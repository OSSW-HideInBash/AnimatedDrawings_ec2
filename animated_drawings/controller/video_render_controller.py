...
class VideoRenderController(Controller):
    ...
    def __init__(self, cfg: ControllerConfig, scene: Scene, view: View) -> None:
        ...
        self.render_start_time: (
            float  # track when we started to render frames (for performance stats)
        )
    ...
    def _cleanup_after_run_loop(self) -> None:
        logging.info(
            f"Rendered {self.frames_rendered} frames in {time.time()-self.run_loop_start_time} seconds."
        )
        self.view.cleanup()
...
class MP4Writer(VideoWriter):
    ...
    def __init__(self, controller: VideoRenderController) -> None:
        ...
        self.video_writer = cv2.VideoWriter(
            ...
            fourcc,
            frame_rate,
            (controller.video_width, controller.video_height),
        )
