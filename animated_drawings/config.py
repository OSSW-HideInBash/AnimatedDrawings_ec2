...
class Config:
    ...
    def __init__(self, user_mvc_cfg_fn: str) -> None:
        ...
        logging.info(
            f"Using user-specified mvc config file located at {user_mvc_cfg_p.resolve()}"
        )
        ...
        self.scene: SceneConfig = SceneConfig(
            {**base_cfg["scene"], **user_cfg["scene"]}
        )
        self.controller: ControllerConfig = ControllerConfig(
            {**base_cfg["controller"], **user_cfg["controller"]}
        )
        ...
        if self.controller.mode == "interact":
            try:
                assert (
                    self.view.use_mesa is False
                ), "cannot use interactive controller when USE_MESA is True"
...
class SceneConfig:
    ...
    def __init__(self, scene_cfg: dict) -> None:
        ...
        self.animated_characters: List[
            Tuple[CharacterConfig, RetargetConfig, MotionConfig]
        ] = []
...
class ViewConfig:
    ...
    def __init__(self, view_cfg: dict) -> None:  # noqa: C901
        ...
        try:
            ...
            assert isinstance(
                self.background_image, (NoneType, str)
            ), "type not NoneType or str"
        except (AssertionError, ValueError) as e:
    ...
    def __init__(self, controller_cfg: dict) -> None:
        ...
        try:
            ...
            assert self.mode in (
                "interactive",
                "video_render",
            ), "mode not interactive or video_render"
        except (AssertionError, ValueError) as e:
        ...
        try:
            self.keyboard_timestep: Union[float, int] = controller_cfg[
                "KEYBOARD_TIMESTEP"
            ]
            assert isinstance(
                self.keyboard_timestep, (float, int)
            ), "is not floar or int"
        ...
        try:
            self.output_video_path: Union[None, str] = controller_cfg[
                "OUTPUT_VIDEO_PATH"
            ]
            assert isinstance(
                self.output_video_path, (NoneType, str)
            ), "type is not None or str"
        ...
        try:
            self.output_video_codec: Union[None, str] = controller_cfg[
                "OUTPUT_VIDEO_CODEC"
            ]
            assert isinstance(
                self.output_video_codec, (NoneType, str)
            ), "type is not None or str"
        except (AssertionError, ValueError) as e:
    ...
    def __init__(self, char_cfg_fn: str) -> None:  # noqa: C901
        ...
        try:
            ...
            for joint in char_cfg["skeleton"]:
                ...
                loc_y: float = loc[1] / self.img_dim + (
                    1 - self.img_height / self.img_dim
                )  # height
                ...
                assert isinstance(
                    parent, (NoneType, str)
                ), "parent must be str or NoneType"

                self.skeleton.append(
                    {"loc": [loc_x, loc_y], "name": name, "parent": parent}
                )
        except AssertionError as e:
...
class MotionConfig:
    ...
    def __init__(self, motion_cfg_fn: str) -> None:  # noqa: C901
        ...
        try:
            ...
            assert isinstance(
                self.end_frame_idx, (NoneType, int)
            ), "type not NoneType or int"
            if isinstance(self.end_frame_idx, int):
                assert (
                    self.end_frame_idx >= self.start_frame_idx
                ), "end_frame_idx must be > start_frame_idx"
        except (AssertionError, ValueError) as e:
        ...
        try:
            ...
            assert isinstance(
                self.frame_time, (NoneType, float)
            ), "is not None or float"
        except (AssertionError, ValueError) as e:
        ...
        try:
            ...
            assert isinstance(
                self.groundplane_joint, str
            ), "groundplane joint must be str"
        except (AssertionError, ValueError) as e:
        ...
        try:
            self.forward_perp_joint_vectors: List[Tuple[str, str]] = motion_cfg[
                "forward_perp_joint_vectors"
            ]
            assert (
                len(self.forward_perp_joint_vectors) > 0
            ), "forward_perp_joint_vectors len must be > 0"
            for each in self.forward_perp_joint_vectors:
                assert (
                    len(each) == 2
                ), "each list in forrward_perp_joint_vectors must have len = 2"
        except (AssertionError, ValueError) as e:
        ...
        # validate bvh_p
        try:
            self.bvh_p: Path = resolve_ad_filepath(
                motion_cfg["filepath"], "bvh filepath"
            )
        except (AssertionError, ValueError) as e:
...
class RetargetConfig:
    ...
    def __init__(self, retarget_cfg_fn: str) -> None:  # noqa: C901
        ...
        self.bvh_projection_bodypart_groups: List[
            RetargetConfig.BvhProjectionBodypartGroup
        ]
        try:
            self.bvh_projection_bodypart_groups = retarget_cfg[
        ...
        try:
            group_names = [
                group["name"] for group in self.bvh_projection_bodypart_groups
            ]
        ...
        try:
            ...
            for group in self.char_bodypart_groups:
                assert (
                    len(group["bvh_depth_drivers"]) > 0
                ), "bvh_depth_drivers must have at least one joint specified"
        except (AssertionError, ValueError) as e:
        ...
        try:
            ...
            assert (
                len(self.char_bvh_root_offset["bvh_joints"]) > 0
            ), "bvh_joints list must be greater than zero"
            for each in self.char_bvh_root_offset["bvh_joints"]:
                assert len(each) > 0, "each list in bvh_joints must have len > 0"

            assert (
                len(self.char_bvh_root_offset["char_joints"]) > 0
            ), "char_joints list must be greater than zero"
            for each in self.char_bvh_root_offset["char_joints"]:
                assert len(each) > 0, "each list in char_joints must have len > 0"

            assert isinstance(
                self.char_bvh_root_offset["bvh_projection_bodypart_group_for_offset"],
                str,
        ...
        try:
            self.char_joint_bvh_joints_mapping = retarget_cfg[
            ...
            for key, val in self.char_joint_bvh_joints_mapping.items():
                ...
                assert isinstance(val[0], str) and isinstance(
                    val[1], str
                ), "values must be str"
        except (AssertionError, ValueError) as e:
            msg = f"Error validating char_bvh_root_offset: {e}"
    ...
    def validate_char_and_bvh_joint_names(
        ...
        try:
            for group in self.bvh_projection_bodypart_groups:
                for bvh_joint_name in group["bvh_joint_names"]:
                    assert (
                        bvh_joint_name in bvh_joint_names
                    ), f"bvh_joint_name not valid: {bvh_joint_name}"
        except AssertionError as e:
            msg = f"Error validating bvh_projection_bodypart_groups: {e}"
            logging.critical(msg)
            assert False, msg
        ...
        try:
            for group in self.char_bodypart_groups:
                ...
                for bvh_joint_name in group["bvh_depth_drivers"]:
                    assert (
                        bvh_joint_name in bvh_joint_names
                    ), f"bvh_depth_driver joint name invalid: {bvh_joint_name}"

                # check that all char_joints are valid character joints
                for char_joint_name in group["char_joints"]:
                    assert (
                        char_joint_name in char_joint_names
                    ), f"char_joints joint name invalid: {char_joint_name}"
        except AssertionError as e:
            msg = f"Error validating char_bodypart_groups: {e}"
            logging.critical(msg)
            assert False, msg
        ...
        try:
            ...
            for bvh_joint_name_group in self.char_bvh_root_offset["bvh_joints"]:
                for joint_name in bvh_joint_name_group:
                    assert (
                        joint_name in bvh_joint_names
                    ), f"invalid joint name in bvh_joints: {joint_name}"
            ...
            for char_joint_name_group in self.char_bvh_root_offset["char_joints"]:
                for joint_name in char_joint_name_group:
                    assert (
                        joint_name in char_joint_names
                    ), f"invalid joint name in char_joints: {joint_name}"
        except AssertionError as e:
            msg = f"Error validating char_bvh_root_offset: {e}"
            logging.critical(msg)
            assert False, msg
        ...
        try:
            ...
            for char_joint_name in self.char_joint_bvh_joints_mapping.keys():
                assert (
                    char_joint_name in char_joint_names
                ), f"invalid char_joint_name: {char_joint_name}"

            # check that dict values correspond to valid bvh joints
            for (
                bvh_prox_joint_name,
                bvh_dist_joint_name,
            ) in self.char_joint_bvh_joints_mapping.values():
                assert (
                    bvh_prox_joint_name in bvh_joint_names
                ), f"invalid bvh_prox_joint_name: {bvh_prox_joint_name}"
                assert (
                    bvh_dist_joint_name in bvh_joint_names
                ), f"invalid bvh_dist_joint_name: {bvh_dist_joint_name}"
        except AssertionError as e:
        ...
        try:
            for check in self.char_runtime_checks:
                if check[0] == "above":
                    ...
                    assert (
                        joint1_name in char_joint_names
                    ), f"above test joint1_name invalid {joint1_name}"
                    assert (
                        joint2_name in char_joint_names
                    ), f"above test joint2_name invalid {joint2_name}"
        except AssertionError as e:
            msg = f"Error validating char_runtime_checks: {e}"
            logging.critical(msg)
            assert False, msg
