...
class AnimatedDrawingRig(Transform):
    ...
    def __init__(self, char_cfg: CharacterConfig):
        ...
        joints_d = {
            joint["name"]: AnimatedDrawingsJoint(joint["name"], *joint["loc"])
        ...
        def _update_positions(t: Transform):
            ...
            if parent is not None:
                offset = np.subtract(
                    t.get_local_position(), parent.get_world_position()
                )
        ...
        for _, joint in joints_d.items():
            ...
            v2 = Vectors(
                [np.subtract(joint.get_world_position(), parent.get_world_position())]
            )
            v2.norm()
    ...
    def get_joints_2D_positions(self) -> npt.NDArray[np.float32]:
        ...
        return np.array(self.root_joint.get_chain_worldspace_positions()).reshape(
            [-1, 3]
        )[:, :2]

    def _compute_buffer_vertices(
        self, parent: Optional[Transform], pointer: List[int]
    ) -> None:
    ...
    def _set_global_orientations(
        self, joint: AnimatedDrawingsJoint, bvh_orientations: Dict[str, float]
    ) -> None:
        if joint.name in bvh_orientations.keys():
            ...
            rotation_q = Quaternions.from_angle_axis(
                np.array([theta]), axes=Vectors([0.0, 0.0, 1.0])
            )
...
class AnimatedDrawing(Transform, TimeManager):
    ...
    def __init__(
        ...
        self.indices: npt.NDArray[np.int32] = np.stack(
            self.mesh["triangles"]
        ).flatten()  # order in which to render triangles
        ...
        self.arap = ARAP(
            self.rig.get_joints_2D_positions(),
            self.mesh["triangles"],
            self.mesh["vertices"],
        )
    ...
    def _modify_retargeting_cfg_for_character(self):
        ...
        for (
            ...
            joint1_name,
            joint2_name,
        ) in self.retarget_cfg.char_runtime_checks:
            if position_test == "above":
                ...
                if joint2 is None:
                    ...
                    logging.critical(msg)
                    assert False, msg
                target_joint = self.rig.root_joint.get_transform_by_name(
                    target_joint_name
                )
                ...
                if (
                    math.sin(-angle) * target_vector[0]
                    + math.cos(-angle) * target_vector[1]
                ) < 0:
                    logging.info(
                        f"char_runtime_check failed, removing {target_joint_name} from retargeter :{target_joint_name, position_test, joint1_name, joint2_name}"
                    )
                    del self.retarget_cfg.char_joint_bvh_joints_mapping[
                        target_joint_name
                    ]
            else:
    ...
    def _initialize_retargeter_bvh(
        self, motion_cfg: MotionConfig, retarget_cfg: RetargetConfig
    ):
        ...
        retarget_cfg.validate_char_and_bvh_joint_names(
            char_joint_names, bvh_joint_names
        )

        # a shorter alias
        char_bvh_root_offset: RetargetConfig.CharBvhRootOffset = (
            self.retarget_cfg.char_bvh_root_offset
        )
        ...
        for b_joint_group in c_joint_groups:
            while len(b_joint_group) >= 2:
                c_dist_joint = self.rig.root_joint.get_transform_by_name(
                    b_joint_group[1]
                )
                c_prox_joint = self.rig.root_joint.get_transform_by_name(
                    b_joint_group[0]
                )
                ...
                c_limb_length += np.linalg.norm(
                    np.subtract(c_dist_joint_pos, c_prox_joint_pos)
                )
                b_joint_group.pop(0)
        ...
        for b_joint_group in b_joint_groups:
            while len(b_joint_group) >= 2:
                b_dist_joint = self.retargeter.bvh.root_joint.get_transform_by_name(
                    b_joint_group[1]
                )
                b_prox_joint = self.retargeter.bvh.root_joint.get_transform_by_name(
                    b_joint_group[0]
                )
                ...
                b_limb_length += np.linalg.norm(
                    np.subtract(b_dist_joint_pos, b_prox_joint_pos)
                )
                b_joint_group.pop(0)

        # compute character-bvh scale factor and send to retargeter
        scale_factor = float(c_limb_length / b_limb_length)
        projection_bodypart_group_for_offset = char_bvh_root_offset[
            "bvh_projection_bodypart_group_for_offset"
        ]
        self.retargeter.scale_root_positions_for_character(
            scale_factor, projection_bodypart_group_for_offset
        )
        ...
        for char_joint_name, (
            bvh_prox_joint_name,
            bvh_dist_joint_name,
        ) in self.retarget_cfg.char_joint_bvh_joints_mapping.items():
            self.retargeter.compute_orientations(
                bvh_prox_joint_name, bvh_dist_joint_name, char_joint_name
            )
    ...
    def update(self):
        ...
        frame_orientations, joint_depths, root_position = (
            self.retargeter.get_retargeted_frame_data(self.get_time())
        )
        ...
        control_points: npt.NDArray[np.float32] = (
            self.rig.get_joints_2D_positions() - root_position[:2]
        )
    ...
    def _set_draw_indices(self, joint_depths: Dict[str, float]):
        ...
        for idx, bodypart_group_dict in enumerate(
            self.retarget_cfg.char_bodypart_groups
        ):
            bodypart_depth: np.float32 = np.mean(
                [
                    joint_depths[joint_name]
                    for joint_name in bodypart_group_dict["bvh_depth_drivers"]
                ]
            )
        ...
        for idx, dist in _bodypart_render_order:
            intra_bodypart_render_order = (
                1 if dist > 0 else -1
            )  # if depth driver is behind plane, render bodyparts in reverse order
            for joint_name in self.retarget_cfg.char_bodypart_groups[idx][
                "char_joints"
            ][::intra_bodypart_render_order]:
                indices.append(
                    self.joint_to_tri_v_idx.get(
                        joint_name, np.array([], dtype=np.int32)
                    )
                )
    ...
    def _initialize_joint_to_triangles_dict(self) -> None:  # noqa: C901
        ...
        shortest_distance = np.full(
            self.mask.shape, 1 << 12, dtype=np.int32
        )  # to nearest joint
        closest_joint_idx = np.full(
            self.mask.shape, -1, dtype=np.int8
        )  # track joint idx nearest each point
        ...
        joint_name_to_idx: List[str] = [
            joint["name"] for joint in self.char_cfg.skeleton
        ]

        # seed generation
        heap: List[Tuple[float, Tuple[int, Tuple[int, int]]]] = (
            []
        )  # [(dist, (joint_idx, (x, y))]
        for _, joint in joints_d.items():
            ...
            seeds_xy = (
                self.img_dim
                * np.linspace(dist_joint_xy, prox_joint_xy, num=20, endpoint=False)
            ).round()
            heap.extend(
                [
                    (0, (joint_idx, tuple(seed_xy.astype(np.int32))))
        ...
        logging.info(
            f"Finished joint -> mask pixel BFS in {time.time() - start_time} seconds"
        )

        # create map between joint name and triangle centroids it is closest to
        joint_to_tri_v_idx_and_dist: DefaultDict[
            str, List[Tuple[npt.NDArray[np.int32], np.int32]]
        ] = defaultdict(list)
        for tri_v_idx in self.mesh["triangles"]:
            tri_verts = np.array([self.mesh["vertices"][v_idx] for v_idx in tri_v_idx])
            centroid_x, centroid_y = list(
                (tri_verts.mean(axis=0) * self.img_dim).round().astype(np.int32)
            )
            tri_centroid_closest_joint_idx: np.int8 = closest_joint_idx[
                centroid_x, centroid_y
            ]
            dist_from_tri_centroid_to_bone: np.int32 = shortest_distance[
                centroid_x, centroid_y
            ]
            joint_to_tri_v_idx_and_dist[
                joint_name_to_idx[tri_centroid_closest_joint_idx]
            ].append((tri_v_idx, dist_from_tri_centroid_to_bone))
    ...
    def _load_mask(self) -> npt.NDArray[np.uint8]:
        ...
        try:
            _mask: npt.NDArray[np.uint8] = cv2.imread(
    ...
    def _load_txtr(self) -> npt.NDArray[np.uint8]:
        ...
        try:
            _txtr: npt.NDArray[np.uint8] = cv2.imread(
                str(txtr_p), cv2.IMREAD_IGNORE_ORIENTATION | cv2.IMREAD_UNCHANGED
            ).astype(np.uint8)
    ...
    def _generate_mesh(self) -> None:
        try:
            contours: List[npt.NDArray[np.float64]] = measure.find_contours(
                self.mask, 128
            )
        except Exception as e:
        ...
        outside_vertices: npt.NDArray[np.float64] = measure.approximate_polygon(
        ...
        vertices: npt.NDArray[np.float32] = np.concatenate(
            [outside_vertices, inside_vertices]
        ).astype(np.float32)
    ...
    def _initialize_vertices(self) -> None:
        ...
        self.vertices[:, :2] = self.arap.solve(
            self.rig.get_joints_2D_positions()
        ).reshape([-1, 2])
    ...
    def _draw(self, **kwargs):
        ...
        if kwargs["viewer_cfg"].draw_ad_mesh_lines:
            ...
            color_black_loc = GL.glGetUniformLocation(
                kwargs["shader_ids"]["color_shader"], "color_black"
            )
