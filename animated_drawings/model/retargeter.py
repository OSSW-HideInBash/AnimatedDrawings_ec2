...
class Retargeter:
    ...
    def __init__(self, motion_cfg: MotionConfig, retarget_cfg: RetargetConfig) -> None:
        ...
        # instantiate the bvh
        try:
            self.bvh = BVH.from_file(
                str(motion_cfg.bvh_p),
                motion_cfg.start_frame_idx,
                motion_cfg.end_frame_idx,
            )
        ...
        self.forward_perp_vector_joint_names: List[Tuple[str, str]] = (
            motion_cfg.forward_perp_joint_vectors
        )
        ...
        if motion_cfg.up == "+y":
            ...
            self.bvh.set_rotation(
                Quaternions.from_euler_angles("yx", np.array([-90.0, -90.0]))
            )
        else:
        ...
        skeleton_fwd: Vectors = self.bvh.get_skeleton_fwd(
            self.forward_perp_vector_joint_names
        )
        q: Quaternions = Quaternions.rotate_between_vectors(
            skeleton_fwd, Vectors([1.0, 0.0, 0.0])
        )
        ...
        try:
            groundplane_joint = self.bvh.root_joint.get_transform_by_name(
                motion_cfg.groundplane_joint
            )
            assert isinstance(
                groundplane_joint, Joint
            ), f"could not find joint by name: {motion_cfg.groundplane_joint}"
        except Exception as e:
        ...
        self.character_start_loc: npt.NDArray[np.float32] = np.array(
            retarget_cfg.char_start_loc, dtype=np.float32
        )
        ...
        for joint_projection_group in retarget_cfg.bvh_projection_bodypart_groups:
            ...
            projection_plane = self._determine_projection_plane_normal(
                group_name, joint_names, projection_method
            )
            self.joint_group_name_to_projection_plane[
                joint_projection_group["name"]
            ] = projection_plane
        ...
        self.bvh_joint_to_projection_depth: Dict[str, npt.NDArray[np.float32]] = (
    ...
    def _compute_normalized_joint_positions_and_fwd_vectors(self) -> None:
        ...
        self.joint_positions = np.empty(
            [self.bvh.frame_max_num, 3 * self.bvh.joint_num], dtype=np.float32
        )
        ...
        for frame_idx in range(self.bvh.frame_max_num):
            ...
            self.joint_positions[frame_idx] = (
                self.bvh.root_joint.get_chain_worldspace_positions()
            )
            self.fwd_vectors[frame_idx] = self.bvh.get_skeleton_fwd(
                self.forward_perp_vector_joint_names
            ).vs[0]

        # reposition over origin
        self.bvh_root_positions = self.joint_positions[:, :3]
        self.joint_positions = self.joint_positions - np.tile(
            self.bvh_root_positions, [1, len(self.bvh_joint_names)]
        )

        # compute angle between skeleton's forward vector and x axis
        v1 = np.tile(
            np.array([1.0, 0.0], dtype=np.float32),
            reps=(self.joint_positions.shape[0], 1),
        )
        v2 = self.fwd_vectors
        ...
        for idx in range(self.joint_positions.shape[0]):
            ...
            rotated_joints: npt.NDArray[np.float32] = (
                rot_mat @ self.joint_positions[idx].reshape([-1, 3]).T
            )
            self.joint_positions[idx] = rotated_joints.T.reshape(
                self.joint_positions[idx].shape
            )

    def _determine_projection_plane_normal(
        self, group_name: str, joint_names: List[str], projection_method: str
        ...
        if projection_method == "frontal":
            logging.info(
                f"{group_name} projection_method is {projection_method}. Using {x_axis}"
            )
            return x_axis
        elif projection_method == "sagittal":
            logging.info(
                f"{group_name} projection_method is {projection_method}. Using {z_axis}"
            )
            return z_axis
        elif projection_method == "pca":
            logging.info(
                f"{group_name} projection_method is {projection_method}. Running PCA on {joint_names}"
            )
        ...
        joints_idxs = [
            self.bvh_joint_names.index(joint_name) for joint_name in joint_names
        ]
        ...
        pc3: npt.NDArray[np.float32] = pca.components_[
            2
        ]  # pyright: ignore[reportGeneralTypeIssues]

        # see if it is closer to the x axis or z axis
        x_cos_sim: float = np.dot(x_axis, pc3) / (
            np.linalg.norm(x_axis) * np.linalg.norm(pc3)
        )
        z_cos_sim: float = np.dot(z_axis, pc3) / (
            np.linalg.norm(z_axis) * np.linalg.norm(pc3)
        )
    ...
    def scale_root_positions_for_character(
        self, char_to_bvh_scale: float, projection_bodypart_group_for_offset: str
    ) -> None:
        """
        ...
        """
        try:
            projection_plane = self.joint_group_name_to_projection_plane[
                projection_bodypart_group_for_offset
            ]
        except Exception as e:
        ...
        self.char_root_positions = np.empty(
            [self.bvh_root_positions.shape[0], 2], dtype=np.float32
        )
        self.char_root_positions[0] = [0, 0]
        for idx in range(1, self.bvh_root_positions.shape[0]):

            if np.array_equal(
                projection_plane, np.array([0.0, 0.0, 1.0])
            ):  # if sagittal projection
                v1 = self.fwd_vectors[idx]  # we're interested in forward motion
            else:  # if frontal projection
                v1 = self.fwd_vectors[idx][::-1] * np.array(
                    [-1, 1, -1]
                )  # we're interested in lateral motion
            ...
            self.char_root_positions[idx, 0] = self.char_root_positions[
                idx - 1, 0
            ] + char_to_bvh_scale * np.dot(
                v1, delta
            )  # x
            self.char_root_positions[idx, 1] = (
                self.char_root_positions[idx - 1, 1] + char_to_bvh_scale * delta[1]
            )  # y

    def compute_orientations(
        self, bvh_prox_joint_name: str, bvh_dist_joint_name: str, char_joint_name: str
    ) -> None:
        """
        ...
        if (
            dist_joint is None
            or not isinstance(dist_joint, Joint)
            or dist_joint.name is None
        ):
        ...
        if (
            prox_joint is None
            or not isinstance(prox_joint, Joint)
            or prox_joint.name is None
        ):
            msg = "joint {bvh_prox_joint_name} has no parent joint, therefore no bone orientation. Returning zero"
            logging.info(msg)
            self.char_joint_to_orientation[char_joint_name] = np.zeros(
                self.joint_positions.shape[0], dtype=np.float32
            )
            return

        # get joint xyz locations
        dist_joint_idx = self.bvh_joint_names.index(dist_joint.name)
        dist_joint_xyz = self.joint_positions[
            :, 3 * dist_joint_idx : 3 * (dist_joint_idx + 1)
        ]

        prox_joint_idx = self.bvh_joint_names.index(prox_joint.name)
        prox_joint_xyz = self.joint_positions[
            :, 3 * prox_joint_idx : 3 * (prox_joint_idx + 1)
        ]
        ...
        try:
            projection_plane_normal = self.joint_to_projection_plane[
                bvh_dist_joint_name
            ]
        except Exception:
        ...
        # project bone onto 2D plane
        if np.array_equal(projection_plane_normal, x_axis):
            projected_bone_xy = np.stack(
                (-bone_vector[:, 2], bone_vector[:, 1]), axis=1
            )
        elif np.array_equal(projection_plane_normal, z_axis):
        ...
        projected_bone_xy /= np.expand_dims(
            np.linalg.norm(projected_bone_xy, axis=1), axis=-1
        )  # norm vector
        y_axis = np.tile(np.array([0.0, 1.0]), reps=(projected_bone_xy.shape[0], 1))

        at1 = np.arctan2(
            projected_bone_xy[:, 1], projected_bone_xy[:, 0], dtype=np.float32
        )
    ...
    def get_retargeted_frame_data(
        ...
        if self.bvh.frame_max_num <= frame_idx:
            logging.info(
                f"invalid frame_idx ({frame_idx}), replacing with last frame {self.bvh.frame_max_num-1}"
            )
            frame_idx = self.bvh.frame_max_num - 1

        orientations = {
            key: val[frame_idx] for (key, val) in self.char_joint_to_orientation.items()
        }

        joint_depths = {
            key: val[frame_idx]
            for (key, val) in self.bvh_joint_to_projection_depth.items()
        }

        root_position = np.array(
            [
                self.char_root_positions[frame_idx, 0],
                self.char_root_positions[frame_idx, 1],
                0.0,
            ],
            dtype=np.float32,
        )
        root_position += (
            self.character_start_loc
