...
class ARAP:
    ...
    def __init__(
        ...
        self.A1: npt.NDArray[np.float32] = np.zeros(
            [2 * (self.edge_num + self.pin_num), 2 * self.vert_num], dtype=np.float32
        )
        G: npt.NDArray[np.float32] = np.zeros(
            [2 * self.edge_num, 2 * self.vert_num], dtype=np.float32
        )  # holds edge rotation calculations
        ...
        for k, (vi_idx, vj_idx) in enumerate(self.e_v_idxs):
            ...
            self.A1[2 * k : 2 * (k + 1), 2 * vi_idx : 2 * (vi_idx + 1)] = -np.identity(
                2
            )
            ...
            e = np.array([[e_kx, e_ky], [e_ky, -e_kx]], np.float32)

            edge_matrix = np.hstack(
                [
                    np.tile(-np.identity(2), (len(e_vnbr_idxs) - 1, 1)),
                    np.identity(2 * (len(e_vnbr_idxs) - 1)),
                ]
            )
            ...
            for h_offset, v_idx in enumerate(e_vnbr_idxs):
                self.A1[2 * k : 2 * (k + 1), 2 * v_idx : 2 * (v_idx + 1)] -= h[
                    :, 2 * h_offset : 2 * (h_offset + 1)
                ]
                G[2 * k : 2 * (k + 1), 2 * v_idx : 2 * (v_idx + 1)] = g[
                    :, 2 * h_offset : 2 * (h_offset + 1)
                ]
        ...
        for pin_idx, pin_bc in enumerate(pins_bc):
            for v_idx, v_w in pin_bc:
                self.A1[2 * self.edge_num + 2 * pin_idx, 2 * v_idx] = (
                    self.w * v_w
                )  # x component
                self.A1[2 * self.edge_num + 2 * pin_idx + 1, 2 * v_idx + 1] = (
                    self.w * v_w
                )  # y component

        A2_top: npt.NDArray[np.float32] = np.zeros(
            [self.edge_num, self.vert_num], dtype=np.float32
        )
        for k, (vi_idx, vj_idx) in enumerate(self.e_v_idxs):
            A2_top[k, vi_idx] = -1
            A2_top[k, vj_idx] = 1

        A2_bot: npt.NDArray[np.float32] = np.zeros(
            [self.pin_num, self.vert_num], dtype=np.float32
        )
        for pin_idx, pin_bc in enumerate(pins_bc):
            for v_idx, v_w in pin_bc:
    ...
    def solve(self, pins_xy_: npt.NDArray[np.float32]) -> npt.NDArray[np.float64]:
        ...
        pins_xy: npt.NDArray[np.float32] = pins_xy_[
            self.pin_mask
        ]  # pyright: ignore[reportGeneralTypeIssues]

        assert len(pins_xy) == self.pin_num

        self.b1: npt.NDArray[np.float64] = np.hstack(
            [
    ...
    def _xy_to_barycentric_coords(
        ...
        tv_locs: npt.NDArray[np.float32] = np.asarray(
            [vertices[t].flatten() for t in triangles]
        )  # triangle->vertex locations, [T x 6] array
        ...
        for p_xy in points:
            ...
            if not len(containing_t_idxs):
                on_triangle_perimeter = np.bitwise_and(
                    np.bitwise_and(a >= 0, b >= 0), a + b <= 1
                )
            ...
            uvw = self._get_barycentric_coords(
                p_xy, a_xy, b_xy, c_xy
    ...
    def _get_barycentric_coords(
        ...
        v: npt.NDArray[np.float32] = (
            d11 * d20 - d01 * d21
        ) / denom  # pyright: ignore[reportGeneralTypeIssues]
